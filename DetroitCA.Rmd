...
```{r}
transition.probs <- raster::calc(layers, function (states) {
...
names(transition.probs) <- c('prob.undeveloped', 'prob.low.dev', 'prob.high.dev')

transition.probs$prob.undeveloped <- mask(transition.probs$prob.undeveloped,
                                          layers$pop.density, maskvalue=NA)
transition.probs$prob.low.dev <- mask(transition.probs$prob.low.dev,
                                      layers$pop.density, maskvalue=NA)
transition.probs$prob.high.dev <- mask(transition.probs$prob.high.dev,
                                       layers$pop.density, maskvalue=NA)

save(transition.probs, file='rda/2006transitionProbs.rda')
load(file='rda/2006transitionProbs.rda')
```
...
cols[1] <- '#FFFFFF'
image(transition.probs$prob.undeveloped, axes=FALSE, zlim=c(0, 1),
...
```{r}
image(transition.probs$prob.low.dev, axes=FALSE, zlim=c(0, 1),
      xlab='', ylab='', col=cols, breaks=seq(0, 1, by=0.2))
...
```{r}
image(transition.probs$prob.high.dev, axes=FALSE, zlim=c(0, 1),
      xlab='', ylab='', col=cols, breaks=seq(0, 1, by=0.2))

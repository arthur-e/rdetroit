...
legend('bottomright', fill=rev(cols), title='New Development', bty='n', cex=1.3,
...
# dev.copy(png, '~/Workspace/TermProject/outputs/graphics/plot_2011_observed_from_nlcd.png'); dev.off()
```
...
# dev.copy(png, '~/Workspace/TermProject/outputs/graphics/plot_2011_expert_prediction.png'); dev.off()
```
...
# dev.copy(png, '~/Workspace/TermProject/outputs/graphics/plot_2011_observed_minus_expert.png'); dev.off()
```

```{r}
plot(dev2011 - output.mmhc.2011, axes=FALSE, box=FALSE, col=c(cols[1], cols, cols[3]),
     legend=FALSE, xlab='Easting (meters)', ylab='Northing (meters)')
axis(1); axis(2);
legend('bottomright', legend=c('Overestimated', 'Agreement', 'Underestimated'),
       fill=cols, bty="n", title='Development Prediction', cex=1.3)
title('2011 Learned Graph Prediction Subtracted from Observed Land Cover')
# dev.copy(png, '~/Workspace/TermProject/outputs/graphics/plot_2011_observed_minus_learned.png'); dev.off()
```
...
# dev.copy(png, '~/Workspace/TermProject/outputs/graphics/plot_2011_expert_minus_learned.png'); dev.off()
```
...
# dev.copy(png, paste0('~/Workspace/TermProject/outputs/graphics/plot_', YEAR, '_expert_trans_probs_undeveloped.png')); dev.off()
```
...
# dev.copy(png, paste0('~/Workspace/TermProject/outputs/graphics/plot_', YEAR, '_expert_trans_probs_lowdev.png')); dev.off()
```
...
# dev.copy(png, paste0('~/Workspace/TermProject/outputs/graphics/plot_', YEAR, '_expert_trans_probs_highdev.png')); dev.off()
```

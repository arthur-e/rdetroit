...
# Add class labels
cover2000$cover <- factor(cover2000$cover, labels=c('undeveloped', 'low-development', 'high-development'))
cover2006$cover <- factor(cover2006$cover, labels=c('undeveloped', 'low-development', 'high-development'))
```
...
# Start the clock!
ptm <- proc.time()
pdag2000 <- bnlearn::iamb(cover2000[,(names(cover2000) %in% c('cover', 'T003_001', 'T093_001'))])
elapsed <- proc.time() - ptm; elapsed # Stop the clock
```
...
for (i in seq(3, length(vars), by=1)) {
  for (j in c(1,2,3)) {
    ...
    pdag2000 <- bnlearn::iamb(cover2000[,(names(cover2000) %in% vars[1:i])])
    i.elapsed <- proc.time() - ptm # Stop the clock
    
    ptm <- proc.time() # Start the clock!
    pdag2000 <- bnlearn::fast.iamb(cover2000[,(names(cover2000) %in% vars[1:i])])
...
There are errors when certain variables are combined (possibly due to multicollinearity?). These occur at the highest number of variables tried in combination, $n=9$, and so are removed from subsequent analysis. It is interesting to note that the with the inclusion of $n=9$, the exponential time model fits better than the quadratic (polynomial) time model. Without $n=9$ they both do pretty well. There really isn't enough data to discriminate between them, so we're obliged to go with the quadratic time model as this is supported in the literature.

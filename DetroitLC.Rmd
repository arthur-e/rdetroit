...
pdag2000 <- bnlearn::fast.iamb(cover2000[,(names(cover2000) %in% c('cover', 'T003_001', 'T093_001',
                                                              'T155_001', 'T156_001', 'T156_002',
                                                              'T020_001', 'T020_002', 'T020_003'))])
elapsed <- proc.time() - ptm; elapsed # Stop the clock
```

```{r}
# Timings on a 64-bit Intel® Core™ i7-2600 CPU @ 3.40GHz × 8 with 7.8 GB memory
l.params <- c(3, 3, 3, 5, 5, 5, 7, 7, 7, 9, 9, 9)
l.fast.iamb <- c(0.018, 0.020, 0.018, 0.129, 0.117, 0.113, 0.719, 0.720, 0.717, 8.374, 8.334, 8.139)
l.iamb <- c(0.015, 0.017, 0.013, 0.115, 0.121, 0.117, 0.578, 0.659, 0.557, 8.228, 8.151, 8.235)
...
mean.timings <- dcast(timings, n ~ ., mean, value.var='iamb')
names(mean.timings) <- c('n', 'time')
slope <- with(mean.timings, (time[length(time)] - time[1]) / (n[length(n)] - n[1]))

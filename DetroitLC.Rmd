...
require(plyr)
t2000 <- merge(tracts, survey2000, by='FIPS')
t2006 <- merge(tracts, survey2006, by='FIPS')
```
...
```{r}
library(sp, raster, rgdal)
attr2000 <- readOGR('/usr/local/dev/rdetroit/shp/tracts2000.shp', 'tracts2000')
attr2006 <- readOGR('/usr/local/dev/rdetroit/shp/tracts2006.shp', 'tracts2006')

# Match the projection of the land cover layer
attr2000 <- spTransform(attr2000, raster::crs(dev))
attr2006 <- spTransform(attr2006, raster::crs(dev))

# We'll rasterize just on the tract IDs so we can join any/all attribute(s) to it later
devp <- raster::rasterToPoints(dev, spatial=TRUE)
cover2000 <- sp::over(devp, attr2000)
c2000e <- raster::extract(dev, attr2000)
```
